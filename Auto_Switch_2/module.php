 ...
 public function GetConfigurationForm() {
  ...
  $elements_entry_Zig2MQTT=',
    ...
    $action_entry1='{ "type": "Label", "label": "Bitte die zu steuernde Instanz wählen" },
          { "type": "Button", "label": "An", "onClick": "AutSw2_SetOn($id);" },
          { "type": "Button", "label": "Aus", "onClick": "AutSw2_SetOff($id);" }';
...
public function EventTrigger(int $par,bool $value) {
 ...
 public function RequestAction($ident, $value) {
    if(IPS_SemaphoreEnter('AutoSwitch2_RequestAction', 5000)) {
     $par= IPS_GetParent(($this->GetIDForIdent('Status')));
...
private function CreateTimerVar($ident,$name,$CatID,$pos,$icon,$script,$profil){
    $VarID= IPS_CreateVariable(1);
    IPS_SetName($VarID, $name); // Variable benennen
    IPS_SetPosition($VarID, $pos);
    IPS_SetIcon($VarID, $icon);
...
//    $eventScript="\$id = \$_IPS['TARGET'];\n".'$idp = IPS_GetParent($id);';
    ...
    if($this->ReadPropertyBoolean('SelTimer')) // Falls die TimerOption in den Eigenschaften gewählt wird
     if($T_Switch_Val){//falls der Button in "Konfig" auf true gesetzt wird
        ...
        if(!$PRG1_An_ID){
            $this->CreateTimerVar('PRG1_An','PRG1_An', $CatID, 40,'Clock',$script,'Timer_AutoSw2' );
        }
        else
       ...
       $PRG1_Aus_ID=@IPS_GetObjectIDByIdent('PRG1_Aus', $CatID);
        if(!$PRG1_Aus_ID){
            $this->CreateTimerVar('PRG1_Aus','PRG1_Aus', $CatID, 50,'Clock',$script,'Timer_AutoSw2');
        }
        else
         ...
         if(!$PRG2_An_ID){
             $this->CreateTimerVar('PRG2_An','PRG2_An', $CatID, 60,'Clock',$script,'Timer_AutoSw2');
         }
         else
        ...
        $PRG2_Aus_ID=@IPS_GetObjectIDByIdent('PRG2_Aus', $CatID);
         if(!$PRG2_Aus_ID){
             $this->CreateTimerVar('PRG2_Aus','PRG2_Aus', $CatID, 70,'Clock',$script,'Timer_AutoSw2');
         }
         else
        ...
        if($PRG1_An_ID){
            ...
            $PRG1_AnEventID=@IPS_GetObjectIDByIdent('PRG1_AnEvent', $PRG1_An_ID);
            if($PRG1_AnEventID){
                IPS_SetEventActive($PRG1_AnEventID, FALSE);
            }
        }
